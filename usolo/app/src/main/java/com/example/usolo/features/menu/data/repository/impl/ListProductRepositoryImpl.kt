...
class ListProductRepositoryImpl(
    ...
    override suspend fun getProduct(itemId: Int): Result<ProductData> {
        return try {
            val token = LocalDataSourceProvider.get().load("accesstoken").first()
    ...
    override suspend fun getProductsByProfileId(profileId: Int): Result<List<ProductData>> {
        return try {
            val token = LocalDataSourceProvider.get().load("accesstoken").first()
